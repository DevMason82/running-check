...
class RunningEvaluatorNew {
    static func evaluate(current: WeatherData) -> (grade: RunningGrade, coach: RunningCoach) {
        ...
        let airQualityIndex = Int(current.airQualityIndex) ?? 0
        let pollutants = current.pollutants
    ...
    func determineSeason() -> Season {
        ...
        switch season {
        case .spring:
            return evaluateForSpring(feelsLike, humidity, windSpeed, precipitationProbability, airQualityIndex, pollutants)
        case .summer:
            return evaluateForSummer(feelsLike, humidity, uvIndex, windSpeed, airQualityIndex, pollutants)
        case .autumn:
            return evaluateForAutumn(feelsLike, humidity, windSpeed, precipitationProbability, airQualityIndex, pollutants)
        case .winter:
            return evaluateForWinter(feelsLike, snowVolume, windSpeed, airQualityIndex, pollutants)
        }
    }

    // 봄 평가
    private static func evaluateForSpring(
        _ feelsLike: Double,
        _ humidity: Double,
        _ windSpeed: Double,
        _ precipitationProbability: Double,
        _ airQualityIndex: Int,
        _ pollutants: [String: Double]
    ) -> (RunningGrade, RunningCoach) {
        let pm25 = pollutants["PM2.5"] ?? 0
        let pm10 = pollutants["PM10"] ?? 0
        let isRainy = precipitationProbability > 70
        let isWindy = windSpeed > 12.0
        let isPoorAirQuality = airQualityIndex >= 4 || pm25 > 35 || pm10 > 50
        let isTooCold = feelsLike < 8
        let isTooHot = feelsLike > 25

        if isRainy || isWindy || isTooCold || isTooHot || isPoorAirQuality {
            return (.danger, createDangerCoach(
                rainProbability: isRainy,
                rainVolume: false,
                snowVolume: false,
                cold: isTooCold,
                hot: isTooHot,
        ...
        } else if feelsLike < 12 || humidity > 75 || airQualityIndex == 3 || pm25 > 25 || pm10 > 40 {
    ...
    // 여름 평가
    private static func evaluateForSummer(
        _ feelsLike: Double,
        _ humidity: Double,
        _ uvIndex: Double,
        _ windSpeed: Double,
        ...
        if isTooHot || isHumid || isHighUV || isPoorAirQuality {
            return (.danger, createDangerCoach(
        ...
        } else if feelsLike > 28 || humidity > 75 || airQualityIndex == 3 || pm25 > 25 || pm10 > 40 {
            return (.warning, createWarningCoach(
                cold: false,
                hot: feelsLike > 28,
                humid: humidity > 75,
    ...
    // 가을 평가
    private static func evaluateForAutumn(
        _ feelsLike: Double,
        _ humidity: Double,
        _ windSpeed: Double,
        _ precipitationProbability: Double,
        ...
        if isWindy || isRainy || isCold || isPoorAirQuality {
            return (.danger, createDangerCoach(
                rainProbability: isRainy,
                rainVolume: false,
                snowVolume: false,
        ...
        } else if feelsLike < 15 || humidity > 70 || airQualityIndex == 3 || pm25 > 25 || pm10 > 40 {
            return (.warning, createWarningCoach(
                cold: feelsLike < 15,
                hot: false,
                humid: humidity > 70,
    ...
    // 겨울 평가
    private static func evaluateForWinter(
        _ feelsLike: Double,
        _ snowVolume: Double,
        _ windSpeed: Double,
        _ airQualityIndex: Int,
        ...
        if isSnowy || isCold || isIcyWind || isPoorAirQuality {
            return (.danger, createDangerCoach(
                rainProbability: false,
                rainVolume: false,
                snowVolume: isSnowy,
        ...
        } else if feelsLike < 5 || snowVolume > 1 || airQualityIndex == 3 || pm25 > 25 || pm10 > 40 {
            return (.warning, createWarningCoach(
                cold: feelsLike < 5,
                hot: false,
                humid: false,
                rainVolume: false,
                snowVolume: snowVolume > 1,
                uvIndex: false,
                feelsLike: feelsLike,
                humidity: 0
            ))
    ...
    private static func createDangerCoach(
        ...
        feelsLike: Double, windSpeed: Double, uvValue: Double, airQualityIndex: Int? = nil, pm25: Double? = nil, pm10: Double? = nil
    ) -> RunningCoach {
