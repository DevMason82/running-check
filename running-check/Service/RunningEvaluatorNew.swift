...
class RunningEvaluatorNew {
    ...
    private static func createDangerCoach(
        ...
        if rainProbability {
            ...
            """
            gear = """
            - 방풍 재킷: 바람으로부터 체온 손실 방지.
            - 바람에 날리지 않는 모자: 고정력이 좋은 모자 사용.
            - 보온 장갑: 바람이 손끝을 차갑게 하지 않도록 보호.
            """
            let shoes = """
            - 견고한 러닝화: 강풍에서도 안정적인 발걸음 유지.
            - 방수 및 방풍 러닝화: 바람에 의해 젖은 길에서 보호.
            """
            alternative = """
            ...
            - UV 차단 모자: 직사광선을 차단.
            - 선글라스: 눈 보호.
            - SPF 50+ 자외선 차단제: 피부 보호를 위한 필수품.
            """
            let shoes = """
            - 밝은 색상의 러닝화: 햇빛 흡수를 줄여 열을 덜 받음.
            - 통기성이 좋은 러닝화: 발의 과열을 방지.
            """
            alternative = """
    ...
    private static func createWarningCoach(
        ...
        if cold {
            ...
            """
            alternative = "낮 시간대의 비교적 따뜻한 환경에서 러닝을 하거나, 실내 러닝을 고려하세요."
        } else if hot {
            comment = """
            ...
            """
            alternative = "습도가 비교적 낮은 시간대(아침 또는 저녁)에 러닝을 시도하거나 실내 운동을 고려하세요."
        } else if rainVolume {
            comment = """
